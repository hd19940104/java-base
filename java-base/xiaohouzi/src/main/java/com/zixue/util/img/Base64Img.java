...
public class Base64Img {
	...
	public static void main(String[] args)
	{
		...
		String img2 = GetImageStr("d:\\222.jpg");
		System.out.println(strImg);
		System.out.println(img2);
		System.out.println(strImg.equals(img2));
		GenerateImage(img2, "d:\\111.jpg");
	}
	
	// 将流转为字节数组字符串并做Base64编码
	...
	{//将图片文件转化为字节数组字符串，并对其进行Base64编码处理
		String imgFile = img_path;//待处理的图片
		InputStream in = null;
		byte[] data = null;
		//读取图片字节数组
		try
	...
	public static String GetImageStr(byte[] bAry)
	{
		
		//对字节数组Base64编码
		BASE64Encoder encoder = new BASE64Encoder();
		return encoder.encode(bAry);//返回Base64编码过的字节数组字符串
	}
	
	public static String GetImageStrNew(String img_path)
	{//将图片文件转化为字节数组字符串，并对其进行Base64编码处理
		String imgFile = img_path;//待处理的图片
		InputStream in = null;
		byte[] data = null;
	...
	public static boolean GenerateImage(String imgStr, String img_path)
	{
		...
		try
		{
			...
			//生成jpeg图片
			String imgFilePath = img_path;//新生成的图片
	...
	public static boolean GenerateGrayImage(String imgStr, String img_path)
	{
		//对字节数组字符串进行Base64解码并生成图片
		if (imgStr == null) //图像数据为空
			return false;
	...
	public static boolean GenerateImageNew(String imgStr, String img_path)
	{
		//对字节数组字符串进行Base64解码并生成图片
		if (imgStr == null) //图像数据为空
			return false;
	...
	public static byte[] GenerateImageByteNew(String imgStr)
	{//对字节数组字符串进行Base64解码并生成图片
		if (imgStr == null) //图像数据为空
			return null;
		try
	...
	public static String convertBase64Str(byte[] imgByte)
	{
		//对字节数组Base64编码
		BASE64Encoder encoder = new BASE64Encoder();
		//返回Base64编码过的字节数组字符串
